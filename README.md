...
- 划词后先显示紧凑工具卡，保留原文、译文、解释、批注、高亮和提问入口，并可直接拖动到合适位置
- 提问与批注按需展开为独立编辑器，支持拖动、右下角缩放、最小高度收缩以及跨选区记忆位置和大小
- 右侧问答面板新增清晰的左缘收起把手，点击或向右拖动都能收起，并继续支持从右缘反向拖开
- 工具卡与问答面板出现、收起和回答结束时不再改变 PDF 正文宽度，阅读位置保持稳定
- 完整安装包内置 paperq 2.21.0；0.1.14 和 0.1.15 用户也可通过应用内更新获得本次内核功能

> 新下载的 0.1.16 已内置 paperq 2.21.0，不会重复下载同一增量包。Linux 当前继续使用 0.1.13，新的 Linux 安装包仍需在 Linux 机器单独构建。
